...
{
public:
    VoteCount(uint validVotes);
    VoteCount(uint validVotes, uint blankVotes, uint notaVotes);
    VoteCount(const VoteCount& other);
    ...
    static expected<VoteCount, logic_error> createForcedCount(uint totalVotes, uint validVotes, uint blankVotes, uint notaVotes);
    ...
    expected<void, logic_error> addVotes(uint votes);
...
private:
    ...
    const uint blankVotes;
    const uint notaVotes;
    const bool voteCountForced;
};
